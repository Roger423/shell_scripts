...
# Set dev index list to use for kernel nvmf test, pf index start at 0, vf index of each pf start at 0.
# pf: pf0, pf1, pf2...
...
mtu=<interface mtu> tgt_port=<target port list> nvme_count=<nvme disk count for each function> \
...
Arguments:
    ...
    res_dir:               the directory for fio result file to save to. default is 
                           $TEST_RESULT_DIR.
    
    dir_prefix:            the prefix for sub test result dir. default is $TEST_SUB_DIR_PREFIX.

    fio_test:              whether to execute fio test after setup kernel initiator. default is $FIO_TEST.
...
Examples:
    Setup kernel nvme-of initiator with default value for every parameters.
        bash $0

    Setup kernel nvme-of initiator and fio test parameters:
...
discover_tgt() {
    ...
    for i in "${!TGT_IP_LIST[@]}"; do
        ...
        for ((j = 0; j < $NVME_CNT; j++)); do
            local pt=$((init_pt + j))
            echo "Discover target for $t_ip:$pt"
            discover_cmd="nvme discover -t rdma -a $t_ip -s $pt"
            echo "Discover CMD: $discover_cmd"
            
            # 执行 nvme discover 命令
            dis_output=$(nvme discover -t rdma -a $t_ip -s $pt)
            discover_info=$(printf "%s\n%s" "$discover_info" "$dis_output")
        done
    done
    ...
    while read -r line; do
        echo "$line"
        case "$line" in
            *'traddr:'*) ip=$(echo "$line" | awk '{print $NF}' | xargs) ;;
            *'subnqn:'*) nqn=$(echo "$line" | awk -F': ' '{print $2}' | xargs) ;;
            *'trsvcid:'*) tsid=$(echo "$line" | awk -F': ' '{print $2}' | xargs) ;;
        esac

        # 当获取到完整信息时，判断是否存在重复 target
...
connect_tgt() {
    ...
    for idx in "${!tgt_map_ref[@]}"; do
        echo $SPLIT_LINE
        IFS=' ' read -r tgt_ip tgt_port tgt_nqn <<< "${tgt_map_ref[$idx]}"
        echo "Connect to target --> target IP: $tgt_ip, target port: $tgt_port, target nqn: $tgt_nqn"
        connect_cmd="nvme connect -t rdma -n $tgt_nqn -a $tgt_ip -s $tgt_port -k 7200"
        echo "Connect CMD: $connect_cmd"
        success_str="Connect to ${tgt_ip}:${tgt_port} ${tgt_nqn} success"
        fail_str="Connect to ${tgt_ip}:${tgt_port} ${tgt_nqn} failed"
        $connect_cmd && echo $success_str || echo $fail_str
        nvme list
        echo $SPLIT_LINE
    done
}
...
match_devices() {
    local -n nqn_dev_map_ref=$1
    echo "Matched Devices:"

    # 提取 Subsystem-NQN -> Controller 映射
    declare -A subsys_to_ctrl
    while read -r line; do
        subsys=$(echo "$line" | awk '{print $2}')
        ctrl=$(echo "$line" | awk '{print $3}')
        subsys_to_ctrl[$subsys]=$ctrl
    done < <(nvme list -v | awk '/nvme-subsys/ {print $0}')

    # 提取 Controller -> SN 映射
    declare -A ctrl_to_sn
    while read -r line; do
        ctrl=$(echo "$line" | awk '{print $1}')
        sn=$(echo "$line" | awk '{print $2}')
        ctrl_to_sn[$ctrl]=$sn
    done < <(nvme list -v | awk '/nvme[0-9]+ / {print $0}')

    # 提取 Controller -> Device (Namespace) 映射
    declare -A ctrl_to_device
    while read -r line; do
        device=$(echo "$line" | awk '{print $1}')
        ctrl=$(echo "$line" | awk '{print $NF}')
        ctrl_to_device[$ctrl]=$device
    done < <(nvme list -v | awk '/nvme[0-9]+n[0-9]+/ {print $0}')

    # 构建 Subsystem-NQN -> Device 映射
    for subsys in "${!subsys_to_ctrl[@]}"; do
        ctrl="${subsys_to_ctrl[$subsys]}"
        device="${ctrl_to_device[$ctrl]}"
        nqn_dev_map_ref[$subsys]=$device
    done

    # 构建 SN -> Device 映射
    for ctrl in "${!ctrl_to_sn[@]}"; do
        sn="${ctrl_to_sn[$ctrl]}"
        device="${ctrl_to_device[$ctrl]}"
        nqn_dev_map_ref[$sn]=$device
    done
}

get_nvme_disk() {
    local -n tgt_map_refer=$1
    local -n nqn_map_refer=$2
    local nvme_disk_list=""
    for idx in "${!tgt_map_refer[@]}"; do
        IFS=' ' read -r _ _ t_nqn <<< "${tgt_map_refer[$idx]}"
        local nvme_disk_idx=${nqn_map_refer[$t_nqn]}
        if [ -n $nvme_disk_list ]; then
            nvme_disk_list="${nvme_disk_list}:/dev/${nvme_disk_idx}"
        else
            nvme_disk_list="/dev/${nvme_disk_idx}"
        fi
    done
    nvme_disk_list=$(echo $nvme_disk_list | sed 's/^://')
    echo $nvme_disk_list
}
...
fio_test() {
    ...
    local -n t_map_ref=$1
    local -n n_map_ref=$2
    local dev_list_str=$(get_nvme_disk t_map_ref n_map_ref)
...
disconnect_all_tgt() {
    echo "Disconnect all existing connections..."
    nvme disconnect-all
    echo $SPLIT_LINE
    echo "All nvme list after disconnect all nvme-of connections"
    nvme list
}
...
main() {
    ...
    connect_tgt tgt_map
    echo $SPLIT_LINE
    declare -A nqn_dev_map
    match_devices nqn_dev_map
    echo $SPLIT_LINE
    echo "Target nqn to local dev map:"
    for nqn_key in "${!nqn_dev_map[@]}"; do
        echo "$nqn_key --> ${nqn_dev_map[$nqn_key]}"
    done
    if [ "$FIO_TEST" == true ]; then
        echo $SPLIT_LINE
        fio_test tgt_map nqn_dev_map
        echo $SPLIT_LINE
    fi
