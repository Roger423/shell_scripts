...
function vfio_unbind {
	...
	if [ -e /sys/bus/pci/drivers/virtio-pci ]; then
		echo "Bind device to virtio-pci if driver loaded"
		echo "Binding device: $DEV" 1>&2
		echo "virtio-pci" > /sys/bus/pci/devices/$DEV/driver_override
